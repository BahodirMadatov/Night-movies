const searchLink = document.querySelector('.search__link .icon-reg'),
    ...
    moviesLink = document.querySelectorAll('.main__link'),
    ...
    headerAds = document.querySelector('.header__abs'),
...
function openMenu() {
    ...
    headerAds.classList[bool ? 'add' : 'remove']('active')
...
moviesLink.forEach(item => item.addEventListener('clock', openMainBlock))
...
const hostValue = '324ed71a-45db-46cc-b4bb-886664214e30'
...
function renderHeader(page) {
    db.getTopMovies(page).then(res => {
        ...
        const filmRaiting = res.films[max].rating
        db.getSoloFilm(filmId).then(response => {
            ...
            <div class="header__balls">
                ...
                <div class="header__stars header__year"><span class="icon-solid"></span><strong>${filmRaiting}</strong></div>
            </div>
...
function renderSolo(id) {
    ...
    <div class="solo__content">
        ...
        <ul>
            ...
            <li class="solo__countries">Жфнры: ${genres}</li>
            <li class="solo__countries">Продолжительность: ${solo.filmLenght || ''}</li>
            <li class="solo__countries">Год: ${solo.year || ''}</li>
            <li class="solo__countries">Мировая премьера: ${solo.premierWorld || ''}</li>
            <li class="solo__countries">Возрастной рейтинг: ${solo.raitingAgeLimits || ''}</li>
    ...
    db[fn](page, se).then(data => {
        if(data.films.length > 0) {
           ...
           <div class="some__img">
                ...
                <span class="some__rating">${item.raiting || 0}</span>
           </div>
           ...
           renderPagination(page, data.pagesCount);
            ...
            pagination.innerHTML = '';
            mainBlock.innerHTML = `<p class="undefined">Undefined</p> `
        }
    })
    .then(() => {
